...
ensures positivo(s) == (forall i :: 0<=i<|s| ==> s[i]>0 )
...
ensures iguales(a) == (forall i :: 1<=i<|a| ==> a[i] == a[0])
...
ensures distintos(a) == (forall i :: 0<=i<|a| ==> (forall j :: 0<=j<|a| && i != j ==> a[i] != a[j]))
...
ensures contiguosDistintos(s) == (forall i :: 0<=i<|s|-1 ==> s[i] != s[i+1])
...
ensures	estrictamenteCreciente(s) ==  (forall i :: 0<=i<|s|-1 ==> s[i]<s[i+1])
...
ensures creciente(s) == (forall i,j :: 0<=i<j<|s| ==> s[i]<=s[j])
//ensures creciente(s) == (forall i :: 0<=i<|s|-1 ==> s[i]<=s[i+1])
...
requires p > 1
ensures primo(p) == ( forall i :: 1<i<p ==> p % i != 0 ) 
